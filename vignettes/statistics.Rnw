...
# restrict to cVA 
wt.asp_fg.cVA=subset(fixedUseful,shortGenotype%in%wt.genos&resp.cVA==T & cluster%in%c('aSP-f','aSP-g'))
...
for(cell in rownames(wt.asp_fg.cVA)){
  ...
  for (window in windows){
    wt.asp_fg.cVA[cell,as.character(window)]= 
      mean(OdourResponseFromSpikes(cVAspikes,response=c(window,window+200),base=c(0,2000),freq=T)$cVA)
  }
}
...
Now for the Or67d mutant cells (Figure 2)
